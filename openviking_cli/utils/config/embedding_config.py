...
class EmbeddingConfig(BaseModel):
    ...
    def get_embedder(self):
        ...
        if self.hybrid:
            base_embedder = self._create_embedder(
                self.hybrid.provider.lower(), "hybrid", self.hybrid
            )
            return self.CachedEmbedderProxy(base_embedder)

        if self.dense and self.sparse:
            dense_embedder = self.CachedEmbedderProxy(
                self._create_embedder(self.dense.provider.lower(), "dense", self.dense)
            )
            sparse_embedder = self.CachedEmbedderProxy(
