...
class CandidateMemory:
    ...
    # Precomputed vectors for reuse (optimization)
    _precomputed_vector: Optional[List[float]] = None
    _precomputed_sparse_vector: Optional[dict] = None
...
class MemoryExtractor:
    ...
    async def create_memory(
        ...
        if candidate.category == MemoryCategory.PROFILE:
            ...
            if not payload:
                return None
            user_space = ctx.user.agent_space_name()
            memory_uri = f"viking://agent/{user_space}/memories/profile.md"
            memory = Context(
                uri=memory_uri,
                parent_uri=f"viking://agent/{user_space}/memories",
                is_leaf=True,
        ...
        # Force agent isolation for all categories
        parent_uri = f"viking://agent/{ctx.user.agent_space_name()}/{cat_dir}"
