...
class OpenVikingClient {
  ...
  /** Set agent ID for memory isolation (X-OpenViking-Agent header) */
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    try {
      ...
      // Add X-OpenViking-Agent header for memory isolation
      if (this.currentAgentId) {
        headers.set("X-OpenViking-Agent", this.currentAgentId);
      }
  ...
  private async resolveScopeSpace(scope: ScopeName): Promise<string> {
    ...
    // 直接使用 agentId 作为空间名，便于识别
    const fallbackSpace =
      scope === "user" ? identity.userId : identity.agentId;
        ...
        if (cfg.autoRecall && queryText.length >= 5) {
          ...
          if (!precheck.ok) {
            ...
            try {
              ...
              // Set agent ID for isolation mode - try multiple sources
              let agentId: string | null = null;
              if (cfg.isolationMode === "isolated") {
                // Try ctx.agentId first
                agentId = (ctx as { agentId?: string })?.agentId ?? null;
                // Fallback: try event.agentId
                if (!agentId) {
                  agentId = (event as { agentId?: string })?.agentId ?? null;
                }
                // Fallback: try process.env.OPENCLAW_AGENT_ID
                if (!agentId) {
                  agentId = process.env.OPENCLAW_AGENT_ID ?? null;
                }
                // Fallback: try to get from workspace path
                if (!agentId) {
                  const ctxAny = ctx as Record<string, unknown>;
                  const workspace = typeof ctxAny?.workspace === "string" ? ctxAny.workspace : null;
                  if (workspace) {
                    const match = workspace.match(/workspace[_-]?(\w+)/i);
                    if (match) {
                ...
                api.logger.info?.(`memory-openviking: isolation enabled, agentId=${agentId}`);
              }
              const result = await withTimeout(
                getClient().then((client) => {
                  client.setAgentId(agentId);
                  return client.find(queryText, {
      ...
      api.on("agent_end", async (event, ctx) => {
        ...
        try {
          ...
          if (cfg.isolationMode === "isolated") {
            ...
            api.logger.info?.(`memory-openviking: auto-capture isolation enabled, agentId=${agentId}`);
          }
